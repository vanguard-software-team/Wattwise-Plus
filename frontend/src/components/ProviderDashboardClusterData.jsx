...
import { getClusterInfo, getClusterConsumptionHourly, getClusterConsumptionDaily, getClusterConsumptionMonthly } from '../service/api';
	...
	useEffect(() => {
		const fetchCardsData = async () => {
			try {
				...
				const daily_data = await getClusterConsumptionDaily(clusterInfoData.id, startDate, endDate);
				const current_month = today.getMonth();
				const first_day_of_month = new Date(today.getFullYear(), today.getMonth(), 1);
				const monthly_data = await getClusterConsumptionMonthly(clusterInfoData.id, first_day_of_month, today);
				...
				const prev_monthly_data = await getClusterConsumptionMonthly(clusterInfoData.id, start_date_months, end_date_months);
				...
				const last_month_data = await getClusterConsumptionMonthly(clusterInfoData.id, first_day_of_previous_month, last_day_of_previous_month);
								...
								>
									...
									>
										...
										<Tooltip formatter={(value, name) => [
											value,
											name === 'consumption_kwh' ? 'Cluster Consumption (kwh)' :
												name === 'cost_euro' ? 'Cluster Cost (€)' : name
										]} />
										<YAxis yAxisId="right" orientation="right">
											<Label value="Cluster Cost (€)" angle={90} position="insideRight" />
										</YAxis>
										<Legend formatter={(value) => [
											value === 'consumption_kwh' ? 'Cluster Consumption (kwh)' :
												value === 'cost_euro' ? 'Cluster Cost (€)' : value
										]} />
										<Line
											yAxisId="left"
