...
function refreshToken() {
  ...
  return api
    .post(REFRESH_URL, { refresh: refresh })
    .then((response) => {
...
async function fetchData(endpoint, params = {}) {
  ...
  try {
    ...
    if (error.response) {
      ...
      if (status === 404) {
        return { error: "Not found", detail: error.response.data };
      }
      return { error: "Request failed", detail: error.response.data };
    }
...
async function getAllConsumerPowerSupplyNumbers() {
  return fetchData("/consumer/power-supply-numbers", {});
}
